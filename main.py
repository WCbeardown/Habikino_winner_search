...
st.write('使い方：上の「＞」を押して、名前やチーム名を入力')

st.write('レイティング　入賞者の検索')
st.write('注意）羽曳野レイティングのみで、かつ54回（2015年）以降')

#検索条件入力
name = st.sidebar.text_input("名前入力")
#team = st.sidebar.text_input("チーム名入力")
#検索結果
...
data = {'クラス': ['Aグループ', 'Bグループ', 'Cグループ'],
        '1部優勝': [len(a_1_1), len(b_1_1), len(c_1_1)],
        '1部準優勝': [len(a_1_2), len(b_1_2), len(c_1_2)],
        '1部3位': [len(a_1_3), len(b_1_3), len(c_1_3)],
        '2部優勝': [len(a_2_1), len(b_2_1), len(c_2_1)],
        '2部準優勝': [len(a_2_2), len(b_2_2), len(c_2_2)],
        '2部3位': [len(a_2_3), len(b_2_3), len(c_2_3)]}    
# データフレームを作成する　
